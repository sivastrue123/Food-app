  ...
  return (
    <aside className={`${isCollapsed ? 'w-16' : 'w-64'} bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 border-r border-slate-700/50 flex flex-col shadow-2xl fixed left-0 top-0 bottom-0 z-40 transition-all duration-300 ease-in-out`}>
      ...
      <button
        ...
        className="absolute -right-3 top-4 w-5 h-5 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110 z-50"
      ...
      <div className="p-3 border-b border-slate-700/50">
        {!isCollapsed ? (
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-md bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center shadow-lg shadow-blue-500/50">
              <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
              </svg>
            </div>
            <div>
              <h1 className="text-sm font-bold text-white">ZenHotels</h1>
              <p className="text-[10px] text-slate-400">Multi-Outlet</p>
            </div>
          </div>
        ) : (
      ...
      {/* Navigation */}
      <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        {visibleNav.map((item) => {
          ...
          return (
            <a
              ...
              href={item.href}
              className={`
                flex items-center ${isCollapsed ? 'justify-center' : 'gap-2'} px-2.5 py-2 rounded-md text-sm font-medium transition-all duration-200 relative group
                ${isActive 
      ...
      <div className="p-2 border-t border-slate-700/50 bg-slate-900/50">
        {!isCollapsed ? (
          <>
            <div className="flex items-center gap-2 px-2 py-1.5 mb-1.5 rounded-md bg-slate-800/50">
              <div className="w-7 h-7 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white font-bold text-xs shadow-lg">
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white font-bold text-[10px] shadow-lg group relative">
              {user.username.charAt(0).toUpperCase()}
              <div className="absolute left-full ml-2 px-2 py-1 bg-slate-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap transition-opacity duration-200 z-50">
                {user.username}
              </div>
            ...
            <button
              onClick={handleSignOut}
              className="w-7 h-7 rounded-md bg-slate-800/50 border border-slate-700 text-slate-300 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/50 transition-all duration-200 flex items-center justify-center group relative"
              title="Sign Out"
            >
